...
class YTMTNetBase(BaseModel):
    ...
    def eval(self, data, savedir=None, suffix=None, pieapp=None):
        ...
        with torch.no_grad():
            ...
            output_i = tensor2im(self.output_i[-1]) # i for transmission image
            output_j = tensor2im(self.output_j[-1]) # j for reflection image
            target = tensor2im(self.target_t)
            target_r = tensor2im(self.target_r)

            if self.aligned:
                res = index.quality_assess(output_i, target) # quality is measured using only the transmission image
...
class YTMTNetModel(YTMTNetBase):
    ...
    def get_loss(self, out_l, out_r):
        if self.opt.lambda_gan > 0:
            loss_G_GAN = self.loss_dic['gan'].get_g_loss(self.netD, self.input, out_l[-1], self.target_t) * self.opt.lambda_gan
        else:
            loss_G_GAN = torch.zeros(1).cuda()

        loss_icnn_pixel = self.loss_dic['t_pixel'].get_loss(out_l, self.target_t)
        # output left corresponds to target transmission image (i for transmission)
        loss_rcnn_pixel = self.loss_dic['r_pixel'].get_loss(out_r, self.target_r)
        loss_icnn_vgg = self.loss_dic['t_vgg'].get_loss(out_l[-1], self.target_t) * self.opt.lambda_vgg

        return loss_G_GAN, loss_icnn_pixel, loss_rcnn_pixel, loss_icnn_vgg

    def backward_G(self):
        # Make it a tiny bit fasterave
        for p in self.netD.parameters():
        ...
        self.loss_exclu = self.exclusion_loss(self.output_i[-1], self.output_j[-1], 3) * self.opt.lambda_excl
        # exclusion loss

        if self.opt.lambda_reco:
            self.loss_recons = self.loss_dic['recons'](self.output_i, self.output_j, self.input) * self.opt.lambda_reco#0.2 before 2022-5-18
        else:
            self.loss_recons = torch.zeros(1).cuda()
        # reconstruction loss
    ...
    def forward(self):
        # without edge
        # input_i = self.input
        input_i = None#self.input
        if self.vgg is not None:
            input_i = self.hyper_column(self.input)
        output_i, output_j = self.net_i(self.input, input_i, fn=self.data_name[0] if self.data_name else None)

        # if self.vgg is not None:
        #     input_i = self.hyper_column(output_i[-1])
        # output_i, output_j = self.net_i(output_i[-1], input_i, fn=self.data_name[0] if self.data_name else None)
    ...
    def optimize_parameters(self):
        ...
        '''newly added gradient clipping 2021-11-23'''
        nn.utils.clip_grad_norm_(self.net_i.parameters(), max_norm=1, norm_type=2)
    ...
    def get_current_errors(self):
        ...
        if self.loss_rcnn_pixel is not None:
            ret_errors['RPixel'] = self.loss_rcnn_pixel.item() # reflection image
        if self.loss_icnn_pixel is not None:
            ret_errors['IPixel'] = self.loss_icnn_pixel.item()  # transmission image
        if self.loss_icnn_vgg is not None:
            ret_errors['VGG'] = self.loss_icnn_vgg.item() # perceptual loss
        if self.loss_G_GAN is not None:
            ret_errors['GAN'] = self.loss_G_GAN.item() # adversarial loss
        if self.loss_exclu is not None:
            ret_errors['Exclu'] = self.loss_exclu.item() # exclussion loss
        if self.loss_recons is not None:
            ret_errors['Recons'] = self.loss_recons.item() # reconstruction loss
    ...
    def exclusion_loss(self, img_T, img_R, level=3, eps=1e-6):
        ...
        loss_gradxy = torch.sum(sum(grad_x_loss) / level) + torch.sum(sum(grad_y_loss) / level)
