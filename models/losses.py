...
class ReconsLoss(nn.Module):
    def __init__(self, edge_recons=True):
        super().__init__()
        # self.criterion = nn.L1Loss() # originals
    ...
    def forward(self, out_l, out_r, input_i):
        ...
        #     content_diff = content_diff + self.criterion(out_l[i] + out_r[i], input_i)
        content_diff = content_diff + self.criterion(out_l[-1] + out_r[-1], input_i)
        if self.edge_recons:
            out_lx, out_ly = compute_gradient(out_l)
            out_rx, out_ry = compute_gradient(out_r)
            input_x, input_y = compute_gradient(input_i)
...
class ContentLoss():
    ...
    def get_loss(self, fakeIm, realIm):
        ...
        for i in range(len(fakeIm)):
            # loss = loss + ((i+1) / len(fakeIm)) * self.criterion(fakeIm[i], realIm)
            # loss = loss + (2 * (math.floor(i / 2) + 1) / len(fakeIm)) * self.criterion(fakeIm[i], realIm)
            loss = loss + 0.1 * self.criterion(fakeIm[i], realIm)
        loss = loss + 0.9 * self.criterion(fakeIm[-1], realIm)
        loss = loss + 0.9 * self.criterion(fakeIm[-2], realIm)
        return loss
...
class ContentLoss0():
    ...
    def get_loss(self, fakeIm, realIm):
        loss = 0
        loss = loss + 1 * self.criterion(fakeIm, realIm)
        return loss
...
class DiscLossRa(DiscLoss):
    ...
    def get_g_loss(self, net, realA, fakeB, realB, pred_real=None):
        if pred_real is None:
            pred_real = net.forward(realB)
        pred_fake = net.forward(fakeB[-1].unsqueeze(0))
    ...
    def get_loss(self, net, realA, fakeB, realB):
        pred_real = net.forward(realB)
        pred_fake = net.forward(fakeB[-1].detach())
...
def init_loss(opt, tensor):
    ...
    loss_dic['t_pixel'] = pixel_loss # transmission loss contains both MSE loss and gradient loss


    r_loss = ContentLoss()
    r_loss.initialize(MultipleLoss([nn.MSELoss()], [1])) # v6

    loss_dic['r_pixel'] = pixel_loss # reflection loss only contains MSE loss
